...
测试环境接口地址： http://gs.eeeecloud.com:8086/
...
### 访问频率控制：
因为客户端有可能发起脚本攻击，导致服务器资源消耗过大。这方面建议合作伙伴做一下应对。<br>
如果访问超过限定频率，会导致服务器返回错误: ACCESS_FREQUENCY_RESTRICT_ERROR(7034)。合作伙伴请注意处理此错误<br>
访问频率配置，默认为每个渠道500次/10秒，此参数在绝大多数情况下够用了。如果确实用户量比较大，可以协商提高配置访问限制参数值。
...
3.实际开发中，对于单个用户来讲，建议避免发起并发调用，最好按队列依次处理。
...
| 7034       |  ACCESS_FREQUENCY_RESTRICT_ERROR     | 访问频率过高 |
